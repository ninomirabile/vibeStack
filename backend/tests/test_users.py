...
async def test_user_profile_and_update(async_client: AsyncClient):
    ...
    # Update profile
    resp = await async_client.patch("/api/v1/users/me", json={"first_name": "Profile"}, headers=headers)
    assert resp.status_code == 200
    data = resp.json()
...
async def test_admin_list_and_delete(async_client: AsyncClient):
    ...
    for user in users:
        if user["email"] == "pytestuser@vibestack.dev":
            resp = await async_client.delete(f"/api/v1/users/{user['id']}", headers=headers)
            assert resp.status_code == 200 